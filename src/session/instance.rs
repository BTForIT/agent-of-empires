...
impl Instance {
    ...
    /// Builds the tmux launch command (shell + cwd + command). Currently only
    /// called from `start_with_size_opts`; the dead-pane respawn path uses
    /// its own hard-coded shell. Returns `None` for cockpit or other modes
    /// where there is no command to launch.
    ///
