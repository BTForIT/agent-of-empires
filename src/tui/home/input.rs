...
impl HomeView {
    ...
    fn dispatch_action_key(
        ...
        match key.code {
            ...
            // `h`/`H` snooze (temporary, auto wakes) and from `d`/`D` delete
            // (now archives by default but exposes destructive opt ins).
    ...
    /// pre-fills profile + AI engine from the instance's current values, and
    /// on submit either restarts in place (no diff) or restarts with the new
    /// (profile, tool) tuple applied. No-op if no session is selected or the
