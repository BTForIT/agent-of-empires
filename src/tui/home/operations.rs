...
impl HomeView {
    ...
    /// and/or swapping the AI tool first.
    ///
    /// `new_profile`: when `Some(p)` and `p` differs from the current
    /// `source_profile`, the session moves between profile storages.
    /// Mirrors the profile-move path in `rename_selected` so a
    /// restart-with-different-profile behaves the same as rename + restart.
    ///
